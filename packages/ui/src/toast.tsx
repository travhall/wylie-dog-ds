...
const ToastClose = React.forwardRef<HTMLButtonElement, ToastCloseProps>(
  ({ className, srText = "Close notification", ...props }, ref) => (
    <button
      ref={ref}
      className={cn(
        "absolute right-2 top-2 p-1 text-(--color-toast-close) opacity-0 transition-opacity hover:text-(--color-toast-close-hover) focus:opacity-100 focus:outline-none focus:ring-2 group-hover:opacity-100",
