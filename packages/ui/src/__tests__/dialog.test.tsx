...
describe('Dialog', () => {
  ...
  describe('Sub-components', () => {
    it('should render DialogHeader', async () => {
      ...
      await waitFor(() => {
        const header = screen.getByText('Dialog Title').closest('.flex.flex-col');
    ...
    it('should render DialogDescription with proper styling', async () => {
      ...
      await waitFor(() => {
        const description = screen.getByText('This is a dialog description');
        expect(description).toHaveClass('text-sm', 'text-[var(--color-text-secondary)]');
      });
    });
  ...
  describe('Edge Cases', () => {
    it('should handle dialog without title', async () => {
      ...
      await waitFor(() => {
        const dialog = screen.getByRole('dialog', { name: 'Dialog without title' });
        expect(dialog).toBeInTheDocument();
      });
    });
    ...
    it('should handle long content with scrolling', async () => {
      const user = userEvent.setup();
      const longContent = Array.from({ length: 50 }, (_, i) => `Paragraph ${i + 1}`);
