 ...
 *
 * Integrates:
 * - usePluginMessages hook for plugin communication
 * - UIState context for UI state management
 * - useGitHubSync hook for GitHub operations
...
function AppInner() {
  ...
  // Plugin Messages Hook - must be created first to pass actions to useGitHubSync
  // We'll use a forward reference pattern by creating placeholder functions
  const [pluginState, pluginActions] = usePluginMessages(
    githubClient,
    async (config) => await handleGitHubConfigTest(config), // forward ref
    async (exportData) => await handleGitHubSync(exportData), // forward ref
    async () => await handleGitHubPull() // forward ref
  );

  // GitHub Sync Hook - now we have pluginActions available
  const {
