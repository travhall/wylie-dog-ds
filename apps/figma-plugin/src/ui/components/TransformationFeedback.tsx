...
import type {
  ...
  TransformationLog,
} from "../../plugin/variables/format-adapter";
...
export function TransformationFeedback({
  adapterResults,
}: TransformationFeedbackProps) {
  ...
  return (
    <div
      className="transformation-feedback"
      style={{
        marginTop: "16px",
        padding: "12px",
    ...
    >
      ...
      {adapterResults.map((result, index) => (
        <div
          key={index}
          style={{
            marginBottom: index < adapterResults.length - 1 ? "16px" : "0",
          }}
        >
          ...
          {result.transformations.length > 0 && (
            <details style={{ marginBottom: "8px" }}>
              ...
              >
                {result.transformations.length} transformation
                {result.transformations.length !== 1 ? "s" : ""} applied
              </summary>
