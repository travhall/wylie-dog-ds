...
export class GitHubClient {
  ...
  async pullTokens(): Promise<PullResult> {
    ...
    try {
      console.log(
        `Pulling tokens from repository: ${this.config.owner}/${this.config.repo}`
      );
      ...
      for (const file of files) {
        ...
        if (file.type === "file" && file.name && file.name.endsWith(".json")) {
          ...
          try {
            ...
            if (!fileResponse.ok) {
              console.warn(
                `Failed to fetch file ${file.name}: ${fileResponse.status}`
              );
              continue;
            }
      ...
      console.log(
        `Successfully pulled ${tokens.length} token files from GitHub`
      );
