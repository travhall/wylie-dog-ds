...
export class MaterialDesignAdapter implements FormatAdapter {
  ...
  detect(data: any): FormatDetectionResult {
    ...
    if (typeof data === "object" && !Array.isArray(data) && data !== null) {
      ...
      const materialPatterns = keys.filter(
        (key) =>
          key.startsWith("sys.") ||
          key.startsWith("ref.") ||
          key.startsWith("comp.") ||
      ...
      const typographyTokens = keys.filter(
        (key) =>
  ...
  normalize(data: any): NormalizationResult {
    ...
    try {
      ...
      console.log(
        `📦 Organized into ${Object.keys(collections).length} collections`
      );
      ...
      errors.push(
        `Material Design normalization failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
      return {
        data: [],
        transformations,
  ...
  private groupMaterialTokens(
    ...
    for (const [tokenPath, tokenData] of Object.entries(data)) {
      ...
      if (tokenPath.startsWith("ref.") || tokenPath.startsWith("md.ref.")) {
        ...
        tokenPath.startsWith("comp.") ||
        tokenPath.startsWith("md.comp.")
      ) {
        collectionName = "Material Components";
      } else if (
        tokenPath.startsWith("sys.") ||
        tokenPath.startsWith("md.sys.")
      ) {
  ...
  private inferMaterialType(tokenPath: string, value: any): string {
    ...
    if (
      tokenPath.includes(".typeface.") ||
      tokenPath.includes(".font-family.")
    ) {
      return "fontFamily";
    }

    if (
      tokenPath.includes(".font-size.") ||
      tokenPath.includes(".type-scale.")
    ) {
      return "fontSize";
    }
    ...
    if (
      tokenPath.includes(".corner.") ||
      tokenPath.includes(".border-radius.")
    ) {
      return "borderRadius";
    }
  ...
  private elevationToShadow(elevation: number): string {
    ...
    return (
      shadowMap[elevation] ||
      `0px ${elevation * 2}px ${elevation * 4}px rgba(0, 0, 0, 0.2)`
    );
  }
