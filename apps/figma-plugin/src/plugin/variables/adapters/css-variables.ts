...
export class CSSVariablesAdapter implements FormatAdapter {
  ...
  detect(data: any): FormatDetectionResult {
    ...
    if (typeof data === "object" && !Array.isArray(data) && data !== null) {
      ...
      if (totalVarCount === 0) {
        ...
        if (hasRootSection) {
          warnings.push(
            "Found :root section but no CSS variables (--property)"
          );
        }
      } else {
  ...
  normalize(data: any): NormalizationResult {
    ...
    try {
      ...
      console.log(
        `📦 Found ${Object.keys(sections).length} CSS variable sections`
      );
      ...
      for (const [sectionName, variables] of Object.entries(sections)) {
        console.log(
          `🔄 Processing section: ${sectionName} (${Object.keys(variables).length} variables)`
        );
        const collection = this.transformSection(
          sectionName,
          variables,
          transformations
        );
      ...
      errors.push(
        `CSS Variables normalization failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
      return {
        data: [],
        transformations,
  ...
  private extractSections(
    ...
    for (const [sectionName, sectionData] of Object.entries(data)) {
      if (typeof sectionData === "object" && sectionData !== null) {
        const variables = this.extractVariablesFromSection(
          sectionData,
          referenceMap
        );
  ...
  private transformSection(
    ...
    for (const [varName, varValue] of Object.entries(variables)) {
      const processedToken = this.transformVariable(
        varName,
        varValue,
        transformations
      );
