    ...
    try {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        for (const [collectionName, collectionData] of Object.entries(collection)) {
          if (!collectionData || typeof collectionData !== 'object' || !('variables' in collectionData)) {
      ...
      errors.push(`Native format processing failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return {
        data: [],
        transformations,
        warnings,
