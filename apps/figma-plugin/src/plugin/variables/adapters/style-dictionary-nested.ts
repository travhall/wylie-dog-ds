...
export class StyleDictionaryNestedAdapter implements FormatAdapter {
  ...
  detect(data: any): FormatDetectionResult {
    ...
    if (typeof data === "object" && !Array.isArray(data) && data !== null) {
      ...
      if (
        topLevelKeys.includes("color") ||
        topLevelKeys.includes("size") ||
        topLevelKeys.includes("font")
      ) {
        confidence += 0.1;
      }
    }
  ...
  normalize(data: any): NormalizationResult {
    ...
    try {
      ...
      const flattenedTokens = this.flattenNestedStructure(
        data,
        transformations
      );
      ...
      if (Object.keys(flattenedTokens).length === 0) {
        errors.push(
          "No valid tokens found in nested Structure Dictionary structure"
        );
        return {
          data: [],
          transformations,
      ...
      const collections = this.groupTokensByCategory(
        flattenedTokens,
        transformations
      );
      ...
      errors.push(
        `Style Dictionary Nested normalization failed: ${error instanceof Error ? error.message : "Unknown error"}`
      );
      return {
        data: [],
        transformations,
