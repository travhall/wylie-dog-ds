...
```bash
# Component implementation (flat structure)
packages/ui/src/component-name.tsx

# Unit tests
packages/ui/src/__tests__/component-name.test.tsx

# Storybook stories (under the appropriate category folder)
apps/storybook/stories/Components/<Category>/component-name.stories.tsx
```
...
// 3. Style variants
// Variant styles reference component tokens exclusively
const variantStyles = {
  default: 'bg-(--color-component-background) text-(--color-component-text)',
  secondary: 'bg-(--color-component-background-secondary) text-(--color-component-text-secondary)',
  outline: 'border border-(--color-component-border) text-(--color-component-text)',
};

// Size styles reference component tokens — never hardcoded Tailwind scale values
const sizeStyles = {
  sm: 'text-(length:--font-size-component-sm) px-(--space-component-padding-x-sm) py-(--space-component-padding-y-sm)',
  md: 'text-(length:--font-size-component-md) px-(--space-component-padding-x-md) py-(--space-component-padding-y-md)',
  lg: 'text-(length:--font-size-component-lg) px-(--space-component-padding-x-lg) py-(--space-component-padding-y-lg)',
};
```
...
describe('ComponentName', () => {
  ...
  it('applies variant styles', () => {
    ...
    expect(element).toHaveClass('bg-(--color-interactive-primary)');
  });
...
Each component is exported from its own file. Add a named export to `packages/ui/package.json` under the `"exports"` field:
