...
6. **Use in Components:**
   ```tsx
   <div className="space-y-(--space-layout-section-gap)">
     {/* Content */}
   </div>
   ```
...
Tokens are emitted with category-based prefixes: `--color-*`, `--space-*`, `--font-size-*`, `--font-weight-*`, `--shadow-*`, `--duration-*`, etc.

```css
:root {
  --color-background-primary: oklch(0.98 0.01 250);
  --space-button-padding-md: 0.625rem 1rem;
...
- See [Design Standards](/docs/contributing-guidelines-design-standards--docs) for token usage in designs
