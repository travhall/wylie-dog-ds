...
export const WithInteractions: Story = {
  parameters: {
    docs: {
      description: {
        story:
          "Demonstrates real user interactions including typing, clearing, focus management, Tab navigation, and validation states. This story tests keyboard input, clearing with backspace, tabbing between fields, and error state validation.",
      },
    },
  },
  render: () => (
  ...
  play: async ({ canvasElement }) => {
    ...
    // Test 3: Clear username with keyboard
    ...
    // Test 4: Tab to next field
    await userEvent.tab();
    expect(emailInput).toHaveFocus();

    // Test 5: Type email address
    await userEvent.type(emailInput, "john@example.com");
    expect(emailInput).toHaveValue("john@example.com");

    // Test 6: Tab to password field
    await userEvent.tab();
    expect(passwordInput).toHaveFocus();

    // Test 7: Type password (should be masked)
    await userEvent.type(passwordInput, "SecurePass123");
    expect(passwordInput).toHaveValue("SecurePass123");
    expect(passwordInput).toHaveAttribute("type", "password");

    // Test 8: Tab to disabled field (should skip to next focusable element)
    await userEvent.tab();
    ...
    // Test 10: Verify placeholder is visible when empty
