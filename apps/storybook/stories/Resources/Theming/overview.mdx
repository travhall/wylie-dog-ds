...
--color-green-600: oklch(0.627 0.17 149.21);
...
--color-text-primary: oklch(0.208 0.04 265.75);
...
Override styles directly on components using token-backed arbitrary values. Never use hardcoded Tailwind scale values like `bg-purple-600` — those don't adapt to dark mode and break the token hierarchy.

```tsx
<Button className="bg-(--color-interactive-primary) hover:bg-(--color-interactive-primary-hover) text-(--color-text-inverse)">
  Custom Styled Button
</Button>
```

For one-off brand colors, define them as CSS variables in your app's globals first, then reference them as tokens here.
...
:root {
  ...
  --color-brand-200: oklch(0.902 0.06 302);
...
The spacing scale follows a numeric naming convention (`--space-010`, `--space-025`, `--space-050`, `--space-100`, `--space-200`, etc.) where each step doubles the previous. Component spacing tokens (padding, gap, radius) reference these primitives via named component tokens — for example `--space-button-padding-md` or `--space-card-radius`.
...
When creating custom themes, maintain WCAG 2.2 AA contrast ratios:
...
/* Poor contrast */
background: oklch(0.95 0.01 240); /* Very light blue */
color: oklch(0.7 0.15 240); /* Medium blue */
...
- **WCAG Guidelines**: [WCAG 2.2](https://www.w3.org/WAI/WCAG22/quickref/)
