...
<Meta title="Resources/Guides/Component Testing" />

# Testing Guide

Learn how to write comprehensive tests for components using Vitest, Testing Library, and axe-core for accessibility testing.
...
- **axe-core** (via `@axe-core/react`) - Automated accessibility testing integrated with Storybook's a11y addon
...
export default defineConfig({
  test: {
    ...
    css: true,
    coverage: {
      // This repo uses istanbul rather than v8 — v8 was found to cause
      // dev-server timeouts (see apps/storybook/vite.config.ts).
      provider: 'istanbul',
      thresholds: {
        statements: 80,
        branches: 70,
...
```typescript
// Accessibility violations are caught via the Storybook a11y addon (axe-core).
// For unit-level a11y assertions in Vitest, use @axe-core/react:
// import { checkA11y } from 'axe-playwright'; // for Playwright tests
...
describe('Button Variants', () => {
  ...
  variants.forEach((variant) => {
    it(`renders ${variant} variant without errors`, async () => {
      const { container } = render(<Button variant={variant}>Button</Button>);

      // Verify the button renders and is accessible
...
**Benefits:**
- ✅ **Single source of truth** - Write once, test everywhere
- ✅ **Reduce duplication** - No need to recreate component setups in tests
- ✅ **Better coverage** - Automatically test all story variants
- ✅ **Faster development** - Stories double as test cases
...
**DO:**
- ✅ Use `composeStories` to leverage all Storybook configuration
- ✅ Test all story variants for comprehensive coverage
- ✅ Run accessibility tests on composed stories
- ✅ Use stories as the base for complex integration tests

**DON'T:**
- ❌ Duplicate story args in test files - use composed stories
- ❌ Skip testing story variants - loop through all of them
- ❌ Forget to call `setProjectAnnotations` in test setup
- ❌ Mix story-based and manual component rendering in the same test file
...
Accessibility audits run automatically in Storybook via the `@storybook/addon-a11y` addon (powered by axe-core). The design system configures it in `error` mode (`a11y: { test: "error" }` in `.storybook/preview.tsx`), meaning the Vitest addon runs the audit on every story and fails the test run/CI on any violation.

To check a11y compliance in CI, use Storybook's test runner with `@storybook/addon-vitest`:

```bash
# Run all story play functions and a11y checks
pnpm --filter storybook test
```

Individual components also run axe in their Vitest unit tests via `@axe-core/react` — see existing `__tests__/` files for examples.
...
✅ **Test accessibility for all states**
```typescript
const { container } = render(<Button disabled>Disabled</Button>);
const results = await axe(container);
expect(results).toHaveNoViolations();
...
// Good: Test functional behavior and accessibility
expect(button).toBeInTheDocument();
...
- [Accessibility Guidelines](?path=/docs/foundations-accessibility-guidelines--docs)
