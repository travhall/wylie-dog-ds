...
Accessibility audits run automatically in Storybook via the `@storybook/addon-a11y` addon (powered by axe-core). The design system configures it in `manual` mode, meaning you trigger the audit from the **Accessibility** panel rather than it running on every render. This avoids false positives during theme transitions.
